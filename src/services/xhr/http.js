...
var request = {
  ...
  */
  makeRequest: function (url, options, failCallback, successCallback) {
    var defaultOptions = {
      url: url,
      type: 'GET',
      dataType: 'json',
      ...
      httpClient.end(function (err, res) {
        ...
        if (res) {
          ...
          if (res.body.code === 200 || res.body.code === 400) {
            ...
            if (res.body === true || res.body === false) {
              resolve(res.body)
            } else {
              Indicator.close()
              Toast(res.body.msg || '操作失败')
            }
          }
        }
      })
  ...
  * @return {Object} description
  */
  fetch: function (endpoint, options, failCallback, successCallback) {
    ...
    return this.makeRequest(this.makeUrl(end), options, failCallback, successCallback)
  }
}
